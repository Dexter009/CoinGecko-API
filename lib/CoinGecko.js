...
class CoinGecko {
  ...
  /**
   * @description Calls to get trending coins on coingecko 
  */
  trending() {
  ...
  get coins() {
    ...
    return {
       ...
       * @param {string} params.category - List of coins with market data based on category
  ...
  get indexes() {
    ...
    return {
       ...
       * @returns {ReturnObject}
       */
      fetch: (indexId, marketId) => {
        //Must have indexId
        // updata must have market id 
  ...
  /**
   * @description Calls related to category endpoints
  */
  get category() {
    ...
    return{
      ...
      */
      ...
      /**
        * @description List all the categories with market data
        * @function category.list()
        * @param {string} params.order - Order results by CoinGecko.ORDER[*]. Valid Values are MARKET_CAP_ASC(default), 
        * MARKET_CAP_DESC, MARKET_CAP_CHANGE_24H_ASC,  MARKET_CAP_CHANGE_24H_DESC, NAME_ASC, NAME_DESC
        * @returns {ReturnObject}
      */
      fetchCategory: () => {
        const path = `/${pathPrefix}`;
